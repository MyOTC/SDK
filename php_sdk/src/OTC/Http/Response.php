...
final class Response
{
    ...
    {
        if ($name && array_key_exists($name, $this->jsonData)){
    ...
    {
        $retString = 'response ' . $this->statusCode . ' ' . $this->duration . ' ';
        if ($this->statusCode == 200){
            $content = json_encode($this->body);
        }else{
            $content = json_encode($this->error);
        }

        return $retString . mb_substr($content , 0 , 500) . '...';
    }
}
